...
def check_anchors(dataset, model, thr=4.0, imgsz=640):
    ...
    wh = torch.tensor(np.concatenate(
        [l[:, 3:5] * s for s, l in zip(shapes, dataset.labels)])).float()  # wh
...
def rescalecordinates(img1_shape, coords, img0_shape, ratio_pad=None):
...
def nms(prediction, conf_thres=0.1, iou_thres=0.6, fast=False, classes=None, agnostic=False):
...
def create_backbone(f='weights/best.pt', s='weights/backbone.pt'):
...
def classifier(x, model, img, im0):
    ...
    for i, d in enumerate(x):  # per image
        if d is not None and len(d):
            ...
            rescalecordinates(img.shape[2:], d[:, :4], im0[i].shape)
...
def plot_distance(people_coords, img, dist_thres_lim=(200, 250)):
    # Plot lines connecting people
    # dictionary to store if a plotted rectangle has already been labelled as high risk
    already_red = dict()
    centers = []
    ...
    for x in x_combs:
        xyxy1, xyxy2 = x[0], x[1]
        cntr1 = ((int(xyxy1[2])+int(xyxy1[0]))//2,
                 (int(xyxy1[3])+int(xyxy1[1]))//2)
        cntr2 = ((int(xyxy2[2])+int(xyxy2[0]))//2,
                 (int(xyxy2[3])+int(xyxy2[1]))//2)
